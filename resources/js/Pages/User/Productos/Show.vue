<template>
    <v-app>
        <Head :errors="errors" />
        <!-- ======= Hero Section ======= -->
        <section id="hero2" style="padding-left: 10%; margin-top: 10%;">
                        ...
                        <div class="flex-direction: row">
                            ...
                            <v-card max-width="400" hover="true">
                                ...
                                <v-card-text class="text--primary">
                                    <div>
                                        <strong>${{ datops.fltPrecio }}</strong>
                                    </div>
...
export default {
    ...
    mounted() {
        if (localStorage.getItem("carritoProds")) {
            try {
                this.carritoProds = JSON.parse(
                    localStorage.getItem("carritoProds")
                );
            } catch (e) {
    ...
    computed: {
        categ: function() {
            if (this.datops.intIdCat == 1) return "Caballero";
            if (this.datops.intIdCat == 2) return "Dama";
            if (this.datops.intIdCat == 3) return "Infantil";
        },
        tipo: function() {
